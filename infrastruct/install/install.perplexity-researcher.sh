...
VERSION="1.0.23" # Could extract from package.json
...
cp "/home/sim/Obsi/Prods/01-pwf/infrastruct/perplexity-researcher-wrapper.sh" "$BIN_DEST/$APP_NAME"
...
if [ "$DEV_MODE" = "false" ]; then
    ...
    cp "$SCRIPT_DIR/perplexity-researcher.1" "$INSTALL_ROOT/usr/share/man/man1/"
    gzip "$INSTALL_ROOT/usr/share/man/man1/perplexity-researcher.1"
...
EOF
    ...
    log_info "You may need to run 'perplexity-researcher update' to rebuild the Docker image."
fi
