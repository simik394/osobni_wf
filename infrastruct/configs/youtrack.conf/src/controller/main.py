...
class YouTrackClient:
    ...
    def get_agiles(self) -> list[dict]:
        ...
        resp = self.session.get(
            ...
            params={'fields': 'id,name,projects(shortName),columnSettings(field(id),columns(presentation)),sprintsSettings(disableSprints),readSharingSettings(permittedGroups(name)),swimlaneSettings(field(name)),colorCoding(id,prototype(id,name),field(id,name))'}
        )
        resp.raise_for_status()
        return resp.json()
