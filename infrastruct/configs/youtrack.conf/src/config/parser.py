...
def parse_drawio_file(path: Union[str, Path]) -> str:
    """
    Parse a Draw.io diagram using the compiled Nim drawio2prolog binary.
    
    Args:
        path: Path to the .drawio XML file
        
    Returns:
        Prolog facts generated by the Nim parser
    """
    import subprocess
    path = Path(path)
    # Search for drawio2prolog binary.
    # 1. Check in the same directory as this parser.py file.
    binary_path = Path(__file__).parent / "drawio2prolog"
    if not binary_path.exists():
        # 2. Check in src/config/drawio2prolog from the current working directory.
        binary_path = Path("src/config/drawio2prolog")
    
    if not binary_path.exists():
        raise FileNotFoundError(
            f"drawio2prolog binary not found. Please compile src/config/drawio2prolog.nim"
        )
    
    logger.debug(f"Executing Nim parser: {binary_path} {path}")
    res = subprocess.run([str(binary_path.absolute()), str(path.absolute())], capture_output=True, text=True)
    if res.returncode != 0:
        raise RuntimeError(f"Nim drawio2prolog parser failed: {res.stderr}")
