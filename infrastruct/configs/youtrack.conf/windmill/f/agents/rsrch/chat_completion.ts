/**
 * Windmill Script: rsrch Chat Completion
 * 
 * OpenAI-compatible chat completion endpoint for rsrch agent.
 * This script is called by the proxy layer to serialize browser interactions.
 * 
 * @param messages Array of chat messages
 * @param model Model to use (gemini-rsrch, gemini-deep-research, perplexity)
 * @param stream Whether to stream (not supported in Windmill, will be ignored)
 * @param session Optional session name for conversation continuity
...
export async function main(
    ...
    // Get rsrch internal endpoint from Windmill resource
    // Windmill uses Deno runtime, so use Deno.env
    const RSRCH_INTERNAL_URL = Deno.env.get('RSRCH_INTERNAL_URL') || 'http://localhost:13030';

    console.log(`📨 Chat Completion via Windmill`);
    console.log(`  Model: ${model}`);
    console.log(`  Messages: ${messages.length}`);
    console.log(`  Session: ${session || 'new'}`);
    ...
    try {
        const response = await fetch(`${RSRCH_INTERNAL_URL}/v1/chat/completions`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(`rsrch API error: ${response.status} ${errorText}`);
        }
