...
case "$1" in
  ...
  notebook)
    ...
    # Use docker exec directly to avoid compose state issues
    docker exec -it perplexity-server npm run notebook -- "$@"
    ;;
  query)
    shift
    if ! check_running; then
         # Run one-off via compose if not running
         docker compose run --rm -e PORT=$PORT perplexity-server npm run query "$@"
    else
         # Use exec
         docker exec -it perplexity-server npm run query "$@"
    fi
    ;;
  batch)
    shift
    ...
    if ! check_running; then
         ...
         docker exec -it perplexity-server npm run batch /tmp/batch.txt
    fi
    ;;
  build)
