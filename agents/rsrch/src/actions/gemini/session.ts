...
/**
 * Lists available Gems.
 */
export async function listGemsAction(
    ctx: UniversalContext,
    deps: GeminiActionDeps
    ...
    const gemsButton = page.locator(selectors.gemini.sidebar.gems).first();
    if (await gemsButton.isVisible().catch(() => false)) {
        await gemsButton.click();
        await page.waitForTimeout(1000);
    } else {
        log('Gems menu not found in sidebar', 'warn');
    }

    const gems: { name: string; url: string | null }[] = [];
    const gemItems = page.locator(selectors.gemini.gems.card);
    const count = await gemItems.count();

    for (let i = 0; i < count; i++) {
        const item = gemItems.nth(i);
        const nameEl = item.locator(selectors.gemini.gems.name).first();
        let name = await nameEl.innerText().catch(() => '');
        if (!name) name = await item.innerText().catch(() => '');
        name = name.trim();

        let url = await item.getAttribute('href').catch(() => null);
        if (!url) {
            const link = item.locator('a[href*="/gem/"]').first();
            if (await link.count() > 0) {
...
/**
 * Selects a Gem by name.
 */
export async function selectGemAction(
    ctx: UniversalContext,
    deps: GeminiActionDeps,
    gemName: string
): Promise<boolean> {
    ...
    log(`Selecting Gem: ${gemName}`);
    await ensureSidebarAction(ctx, deps);

    const gemSidebarItem = page.locator('nav').locator(`text="${gemName}"`).first();
    if (await gemSidebarItem.isVisible().catch(() => false)) {
        await gemSidebarItem.click();
        await page.waitForTimeout(1000);
        return true;
    }

    const gemsList = await listGemsAction(ctx, deps);
    const targetGem = gemsList.find(g => g.name.toLowerCase() === gemName.toLowerCase());
    if (targetGem && targetGem.url) {
        log(`Navigating to Gem URL: ${targetGem.url}`);
        const gemUrl = targetGem.url.startsWith('http') ? targetGem.url : ctx.config.urls.gemini + targetGem.url;
        await page.goto(gemUrl);
        await page.waitForTimeout(1000);
        return true;
    }

    log(`Gem not found: ${gemName}`, 'error');
    return false;
}
