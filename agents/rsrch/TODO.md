...
---
crossapp:
- [ ] manage the naming of generated audio files so they can be uniquely identifiable in scope of the notebook.
- [ ] manage the naming of research documents so they can be easily identifiable in the scope of the chat session. the management should be done in the gDocs website.
- [ ] manage the naming of gemini chat sessions.
- [ ] naming should be managed in a way that enables easy semantic recognition. thatt means using combinations of at most three or four letters and numbers. eg three for sessions and two for research documents. Finaly there will be all the documents globally identifiable  while preserving the structure and being easy to refer to each of the objects from any other location. you can not define the doc or audio beforehand. you can only rename the created artifact. which can be really challenging if you upload multiple sources and then you naturally start generating audio overviews, but bacause they can finish in different times so  you can just hardly differ which one is it and how to appropriately rename the audio. you have to deduce the correct assignment from the title of the artifact.
- [ ] the original name of the artifact generated by the ai should NOT be deleted. only prefix it with the constructed id.
