...
func (c *Config) ShouldProcess(path string) (fileType string, should bool) {
	...
	for _, pattern := range c.GlobalExclude {
		...
		// Also check with doublestar-style matching
...
// MatchGlob performs glob matching, handling ** patterns
func MatchGlob(pattern, path string) bool {
	// Simple implementation - check if pattern appears in path
	// For proper ** support, use a dedicated library like doublestar
	...
	if len(pattern) >= 2 && pattern[:2] == "**" {
		...
		// Also check if suffix is at the end
		if len(suffix) > 0 {
