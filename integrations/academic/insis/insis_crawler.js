...
function saveAndDiff(filename, newData) {
    let oldData = null;
    if (fs.existsSync(filename)) {
        try {
            oldData = JSON.parse(fs.readFileSync(filename, 'utf8'));
        } catch (e) {
            console.warn(`Could not parse existing ${filename}, assuming empty.`);
        }
    }

    if (oldData) {
        // Compare existing data with new data to identify newly added or modified items
        const oldSet = new Set(oldData.map(item => JSON.stringify(item)));
        const changes = newData.filter(item => !oldSet.has(JSON.stringify(item)));

        if (changes.length > 0) {
            console.log(`\n🚨 CHANGES DETECTED IN ${filename} 🚨`);
            console.log(`Found ${changes.length} new or modified items:`);
...
(async () => {
    ...
    try {
        ...
        const dumpsDir = path.join(__dirname, '_dumps');
        if (!fs.existsSync(dumpsDir)) {
            fs.mkdirSync(dumpsDir);
        }

        saveAndDiff(path.join(dumpsDir, 'insis_dropboxes.json'), dropboxes);
        console.log(`Saved ${dropboxes.length} Dropboxes to _dumps/insis_dropboxes.json`);
        ...
        saveAndDiff(path.join(dumpsDir, 'insis_exams.json'), exams);
        console.log(`Saved ${exams.length} Exams to _dumps/insis_exams.json`);
        ...
        saveAndDiff(path.join(dumpsDir, 'insis_grades.json'), grades);
        console.log(`Saved ${grades.length} Grade entries to _dumps/insis_grades.json`);
        ...
        saveAndDiff(path.join(dumpsDir, 'insis_schedule_anomalies.json'), anomalies);
        console.log(`Saved ${anomalies.length} Schedule Anomalies to _dumps/insis_schedule_anomalies.json`);
        ...
        saveAndDiff(path.join(dumpsDir, 'insis_subject_profiles.json'), fullProfiles);
        console.log(`Saved ${fullProfiles.length} Detailed Profiles to _dumps/insis_subject_profiles.json`);
