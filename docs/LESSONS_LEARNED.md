...
## Storage Maintenance & Automation (2026-01-23)
- **ClickHouse `system.trace_log` Explosion**: ClickHouse's internal `system.trace_log` can grow indefinitely (e.g., 13GB internal logs vs 600MB user data). This accounts for "missing" disk space when user data seems small.
    - *Fix*: Monitor `system.parts` and truncate `system.trace_log` if it becomes disproportionately large.
- **Automation over Manual Reports**: When a generated report (like a storage analysis) is incomplete or needs correction, DO NOT manually edit the output file. Instead, update the generation script to include the missing data (e.g., adding deep-dive queries). This ensures the knowledge is persisted and reproducible for future runs.
